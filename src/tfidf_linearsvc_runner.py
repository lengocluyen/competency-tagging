"""TF-IDF + One-vs-Rest LinearSVC multi-label baseline.

Compared to the existing svm_runner.py, this version exposes:
- word ngrams 1-2 or 1-3, min_df, max_features
- char ngrams 3-5 option
- uses decision_function scores as confidence and supports top-k prediction

Outputs `linearsvc_results.csv` with fold/UV metrics.
"""
...
def predict_linearsvc(
    ...
    *,
    ...
    scores = clf.decision_function(X)  # [n, n_labels]

    out: List[Dict[str, Any]] = []
    for fid, row in zip(frag_ids, scores):
        row = np.asarray(row)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="TF-IDF + OVR LinearSVC baseline")
    ...
    ap.add_argument("--output_dir", type=Path, default=Path("output_linearsvc_alluvs"))
    ...
    ap.add_argument("--char_ngrams", default="3-5")
    ...
    for fold_idx, (train_uvs, test_uvs) in enumerate(cv_splits):
        ...
        vectorizer, clf = train_linearsvc(
            train_texts,
            train_Y,
        ...
        for uv in test_uvs:
            ...
            scores_uv = clf.decision_function(X_uv) if X_uv is not None else np.zeros((0, len(comp_ids)), dtype=np.float32)
            # In binary edge-cases, sklearn may return shape [n_samples]; coerce.
            if scores_uv.ndim == 1:
                scores_uv = scores_uv.reshape(-1, 1)

            mrr = compute_mrr_from_scores(scores_uv, frag_ids, gold, comp_ids)
            mrr_at_k = compute_mrr_from_scores(scores_uv, frag_ids, gold, comp_ids, at_k=args.top_k)

            frag_preds = predict_linearsvc(
                vectorizer,
                clf,
                fragments,
                comp_ids,
    ...
    out_csv = args.output_dir / "linearsvc_results.csv"
